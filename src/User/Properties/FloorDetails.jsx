...
export const FloorDetails = () => {
  ...
  const singlePropData = () => {
    ...
    axios
      .post("/user/get_single_plan", formdata, {
        headers: { tkn: token },
      })
      .then(function (response) {
        ...
        if (response.data.status === 1) {
          ...
          if (response.data.data.length > 0) {
            ...
            setPtype(response.data.data[0].property_type);
            setItems(
        ...
        setNumPages(response.data.data.length);
  ...
  const itemSubmit2 = async (e) => {
    ...
    // else if (ptype !== "Plot" && itemdata.floor_size === "") {
    //   setErr(29);
    //   setSnack({
    ...
    //     direction: "center",
    //   });
    // } else if (ptype !== "Plot" && itemdata.floor_desc === "") {
    //   setErr(32);
    //   setSnack({
  ...
  const handleSubmit = async (e) => {
    ...
    // formdata.append(`floor_size`, itemdata.floor_size);
    // formdata.append(`f_bedrooms`, itemdata.floor_bedroom);
    // formdata.append(`f_bathrooms`, itemdata.floor_bathroom);
    // formdata.append(`f_desc`, itemdata.floor_desc);
    formdata.append(`plan_image`, planimage.raw);
    await axios
      .post("/user/edit_property_floor_plans", formdata, {
        headers: { tkn: token },
      })
      .then(function (res) {
        console.log("hiiiii");
        if (res.data.status === 1) {
          ...
          handleClose();
          singlePropData();
          // history("/Dashboard/all_properties");
        } else {
          setWait(false);
          setSnack({
            message: res.data.msg,
  ...
  const deletefloor = async (e) => {
    ...
    await axios
      .post("/user/deleteFloor", formdata, {
        headers: { tkn: token },
      })
      .then(function (res) {
        console.log("hiiiii");
                            ...
                            <Grid item xs={12} sm={12} md={12} lg={6} xl={5}>
                              {i.floor_name === "" ? (
                                ...
                                >
                                  ...
                                  {/* <Typography variant="body2" gutterBottom>
                                    Floor Size: {i.floor_size}
                                  </Typography>
                                  <Typography variant="body2" gutterBottom>
                                    No. of Bedrooms: {i.floor_bedroom}
                                  </Typography>
                                  <Typography variant="body2" gutterBottom>
                                    No. of Bathrooms: {i.floor_bathroom}
                                  </Typography> */}
                                  {/* <Typography variant='body2' gutterBottom>
                      Description:  {i.floor_desc}
                      </Typography> */}
                                  <Grid
                                    container
                                    direction="row"
