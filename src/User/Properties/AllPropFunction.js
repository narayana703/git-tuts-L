...
export const usePropertyData =()=>{
    const {propData,setPropData,user,token} = useContext(UserContext);
...
const deletedata=async (id)=>{
  ...
  await axios.post("/user/delete_property", formdata, 
  // {
  //   headers: { tkn: tkn },
  // }
  ).then( function (res){
...
const floorData=async (e)=>{
  ...
  formdata.append("plan_image", floor_img.raw);
   axios.post("/user/add_floor_plans", formdata).then(function (res) {
   if (res.data.status === 1) {
