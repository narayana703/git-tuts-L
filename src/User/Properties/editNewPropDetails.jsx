...
export const EditNewPropertyDetails = () => {
  ...
  const getData = () => {
    ...
    axios.post("/user/get_single_property_new", formdata , {
      headers: { tkn: token },
    }).then(function (res) {
  ...
  const handleSubmit = (e) => {
    ...
    if (user.role === 3 && proj_overview === "") {
    ...
    else if (user.role !== 3 && item.rooms === "") {
      setErr(26)
    ...
    else if (user.role !== 3 && item.bedrooms === "") {
      setErr(27)
    ...
    else if (user.role !== 3 && item.bathrooms === "") {
      setErr(28)
    ...
    else if (user.role !== 3 && item.garages === "") {
      setErr(29)
      ...
      formdata.append("uid", user.user_id);
      axios.post("/user/edit_property_details_id", formdata, {
        headers: { tkn: token },
      }).then(function (res) {
        console.log(res)
        if (res.data.status === 1) {
        ...
        >
          ...
          >
            ...
            {user.role === 3 && <>
              ...
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12} textAlign={"start"} ml={3}>
                Project Details
              </Grid>
              <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                {/* {JSON.stringify(size)} */}
