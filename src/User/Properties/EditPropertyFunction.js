...
console.log(id)
  ...
  const singlePropAgentData = () => {
    ...
    console.log(propId)
    formdata.append('id', localStorage.getItem('rowid'))
    axios.post('/user/get_single_agent', formdata)
      .then(function (response) {
        if (response.data.status === 1) {
...
console.log(feature)
 ...
 formdata.append('images', imageids);
    ...
    await axios.post("/user/edit_property", formdata).then(function (res) {
      console.log("hiiiii");
      if (res.data.status === 1) {
        ...
        history('/user_dashboard/propertydetails')
        setLoading(false);
      } else {
        setSnack({
          message: res.data.msg,
 ...
 console.log(images)
    ...
    await axios.post("/user/edit_property_details", formdata).then(function (res) {
      console.log("hiiiii");
      if (res.data.status === 1) {
 ...
 console.log(images)
    ...
    await axios.post("/user/edit_agent_details", formdata).then(function (res) {
      console.log("hiiiii");
      if (res.data.status === 1) {
...
console.log("=========gani=======")
    ...
    formdata.append("id", localStorage.getItem('rowid'))
    await axios.post("/user/deleteimages", formdata).then(function (res) {
      if (res.data.status === 1) {
        setSnack({
          message: res.data.msg,
          type: 'success',
  ...
  const deleteAgentProfileImage=async (id)=>{
    ...
    await axios.post("/user/deleteprofile_image", formdata).then( function (res){
