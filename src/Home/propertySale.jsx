...
export const SaleProperty = () => {
  ...
  const getData = async () => {
    ...
    await axios.post("/user/get_rent_sale_properties_limit").then(function (res) {
      console.log(res.data.dataSale)
      if (res.data.status === 1) {
        setProject(res.data.dataSale)
        handleClick()
      }
    })
  }
  return (
    <div>
      <Container maxWidth="lg">
        ...
        >
          <Typography variant='h4' sx={{ color: "#060847" }}>Latest Properties For Sale</Typography>
          {/* /our_properties/:search/:status/:type */}
